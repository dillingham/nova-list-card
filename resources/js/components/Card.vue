<template>
    <div :id="card.uri_key" :class="'nova-list-card' + card.classes + ' p-8 bg-white relative'">
        ...
        >
            <div class="truncate left" :class="{ 'w-3/4': card.heading.right }">
                {{ card.heading.left }}
            </div>
            <div class="w-1/4 truncate right" v-if="card.heading.right">
                {{ card.heading.right }}
            </div>
        </div>
        <div class="nova-list-card-body relative">
            ...
            <div v-if="!loading && items.length != 0">
                <router-link
                    :to="{
                        name: 'detail',
                        params: { resourceName: item.resourceName, resourceId: item.resourceId },
                    }"
                ...
                >
                    <div class="flex py-1">
                        <div
                            :class="{
                                'w-full': card.value_column == null,
                                'w-3/4 pr-4': card.value_column != null,
                            }"
                        >
                            <p
                            ...
                            <p
                                class="nova-list-card-subtitle text-80 truncate pr-4"
                                v-if="card.subtitle_enabled"
                            >
                                <span class="pb-2" v-if="card.subtitle_column">{{
                                    item.resource[card.subtitle_column]
                                }}</span>
                                <span class="pb-2" v-else-if="item.subTitle">{{
                                    item.subTitle
                                }}</span>
                            </p>
                            <p
                            ...
                            >
                                {{
                                    timestampValue(
                                        item.resource[card.timestamp_column],
                                        card.timestamp_format
                                    )
                                }}
                            </p>
                            <p
                                ...
                                v-if="
                                    item.aggregate &&
                                        card.relationship + '_' + card.aggregate !=
                                        card.value_column &&
                                        card.relationship + '_' + card.aggregate !=
                                        card.subtitle_column
                                "
                            >
    ...
    methods: {
        formatValue(item, format) {
            ...
            if (this.card.value_formatter == 'timestamp') {
                return this.timestampValue(item.resource[this.card.value_column], format);
            }
        },
        ...
        numerialValue(item) {
            return numerial(item.resource[this.card.value_column]).format(this.card.value_format);
        },
        value(item) {
